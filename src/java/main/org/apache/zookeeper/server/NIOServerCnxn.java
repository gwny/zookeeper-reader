...
public class NIOServerCnxn extends ServerCnxn {
	...
	LinkedBlockingQueue<ByteBuffer> outgoingBuffers = new LinkedBlockingQueue<ByteBuffer>(); // tianyakee：输入的缓冲队列
	...
	/**
	 * This method implements the internals of sendBuffer. We have separated it from
	 * send buffer to be able to catch exceptions when testing.
	 *
	 * @param bb
	 *            Buffer to send.
	 */
		...
		try {
			...
			if (k.isReadable()) {
				...
				if (incomingBuffer.remaining() == 0) {
					...
					if (isPayload) { // not the case for 4letterword tianyakee:不是四字节的命令
						readPayload();
					} else {
	...
	private boolean checkFourLetterWord(final SelectionKey k, final int len) throws IOException {
		...
		if (!ServerCnxn.isKnown(len)) { // tianyakee：判断4字节是否为命令
			return false;
		}
		...
		String cmd = ServerCnxn.getCommandString(len); // tianyakee：获取对应的命令
	 ...
	 * letter word.
	 *
	 * tianyakee：读取前4个字节，判断是否为buffer的长度，还是指定的命令
	 *
	 * @param k
	 *            selection key
	...
	private boolean readLength(SelectionKey k) throws IOException {
		...
		if (!initialized && checkFourLetterWord(sk, len)) { // tianyakee：是指定的命令，不是后面数据的长度
			return false;
		}
		...
		incomingBuffer = ByteBuffer.allocate(len); // tianyakee：重新按照数据的长度分配buffer的大小
		return true;
	}
	...
	private void closeSock() {
		...
		LOG.info("Closed socket connection for client " + sock.socket().getRemoteSocketAddress()
				+ (sessionId != 0 ? " which had sessionid 0x" + Long.toHexString(sessionId)
