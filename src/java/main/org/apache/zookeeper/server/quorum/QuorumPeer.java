...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
	...
	/*
	 * ZKDatabase is a top level member of quorumpeer which will be used in all the
	 * zookeeperservers instantiated later. Also, it is created once on bootup and
	 * only thrown away in case of a truncate message from the leader
	 */
	...
	public static class QuorumServer {
		...
		public QuorumServer(long id, String hostname, Integer port, Integer electionPort, LearnerType type) {
			...
			// tianyakee：根据hostname查找ip地址，赋值this.addr和this.electionAddr
		 ...
		 *
		 * If the DNS lookup fails, this.addr and electionAddr remain unmodified, unless
		 * they were never set. If this.addr is null, then it is set with an unresolved
		 * InetSocketAddress object. this.electionAddr is handled similarly.
		 */
		public void recreateSocketAddresses() {
			...
			try {
				...
				// The hostname has never resolved. Create our InetSocketAddress(es) as
				// unresolved
		 ...
		 * @param timeout
		 *            the time, in milliseconds, before {@link InetAddress#isReachable}
		 *            aborts
		 * @return a reachable IP address. If no such IP address can be found, just
		 *         return the first IP address of the hostname.
		 *
	...
	/*
	 * A peer can either be participating, which implies that it is willing to both
	 * vote in instances of consensus and to elect or become a Leader, or it may be
	 * observing in which case it isn't.
	 * 
	...
	/**
	 * tianyakee:<br>
	 * PARTICIPANT：可以投票和参加leader选举，可以成为leader或follower。<br>
	 * OBSERVER：可以投票，但是不能参加leader选举，可以成为observer。<br>
	 */
	...
	/*
	 * To enable observers to have no identifier, we need a generic identifier at
	 * least for QuorumCnxManager. We use the following constant to as the value of
	 * such a generic identifier.
	 */
	...
	/**
	 * Minimum number of milliseconds to allow for session timeout. A value of -1
	 * indicates unset, use default.
	 */
	protected int minSessionTimeout = -1;

	/**
	 * Maximum number of milliseconds to allow for session timeout. A value of -1
	 * indicates unset, use default.
	 */
	...
	/**
	 * Enables/Disables sync request processor. This option is enabled by default
	 * and is to be used with observers.
	 */
	...
	/**
	 * Whether or not to listen on all IPs for the two quorum ports (broadcast and
	...
	/**
	 * If this is false, quorum peer learner will talk to quorum peer server without
	 * authentication. This can be used while upgrading ZooKeeper server. Defaulting
	 * to false (required).
	 */
	...
	/**
	 * The maximum number of threads to allow in the connectionExecutors thread pool
	 * which will be used to initiate quorum server connections.
	 */
	protected int quorumCnxnThreadsSize = QUORUM_CNXN_THREADS_SIZE_DEFAULT_VALUE;

	/**
	 * Keeps time taken for leader election in milliseconds. Sets the value to this
	 * variable only after the completion of leader election.
	 */
	private long electionTimeTaken = -1;

	/**
	 * @deprecated As of release 3.4.0, this class has been deprecated, since it is
	 *             used with one of the udp-based versions of leader election, which
	 *             we are also deprecating.
	 * 
	 *             This class simply responds to requests for the current leader of
	 *             this node.
	 *             <p>
	 *             The request contains just an xid generated by the requestor.
	 *             <p>
	 *             The response has the xid, the id of this server, the id of the
	 *             leader, and the zxid of the leader.
	 *
	 *
	 */
	@Deprecated
	...
	public void initialize() throws SaslException {
		...
		if (isQuorumSaslAuthEnabled()) {
			...
			authServer = new SaslQuorumAuthServer(isQuorumServerSaslAuthRequired(), quorumServerLoginContext,
					authzHosts);
	...
	/**
	 * This constructor is only used by the existing unit test code. It defaults to
	 * FileLogProvider persistence provider.
	 */
	...
	protected Follower makeFollower(FileTxnSnapLog logFactory) throws IOException {
		return new Follower(this,
				new FollowerZooKeeperServer(logFactory, this, new ZooKeeperServer.BasicDataTreeBuilder(), this.zkDb));
	}

	protected Leader makeLeader(FileTxnSnapLog logFactory) throws IOException {
		return new Leader(this,
				new LeaderZooKeeperServer(logFactory, this, new ZooKeeperServer.BasicDataTreeBuilder(), this.zkDb));
	}

	protected Observer makeObserver(FileTxnSnapLog logFactory) throws IOException {
		return new Observer(this,
				new ObserverZooKeeperServer(logFactory, this, new ZooKeeperServer.BasicDataTreeBuilder(), this.zkDb));
	}
	...
	/**
	 * Check if a node is in the current view. With static membership, the result of
	 * this check will never change; only when dynamic membership is introduced will
	 * this be more useful.
	 */
	...
	/**
	 * Updates leader election info to avoid inconsistencies when a new server tries
	 * to join the ensemble. See ZOOKEEPER-1732 for more info.
	 */
	...
	public QuorumCnxManager createCnxnManager() {
		return new QuorumCnxManager(this.getId(), this.getView(), this.authServer, this.authLearner,
