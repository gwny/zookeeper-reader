	...
	public void parseProperties(Properties zkProp) throws IOException, ConfigException {
		...
		for (Entry<Object, Object> entry : zkProp.entrySet()) {
			...
			if (key.equals("dataDir")) {
				 ...
				 * server列表组成Zookpeer的服务，配置由host:port1:port2组成，port1用于机器之间的通信，port2用于leader选举。
				 */
				int dot = key.indexOf('.');
				...
				if ((parts.length != 2) && (parts.length != 3) && (parts.length != 4)) {
					LOG.error(
							value + " does not have the form host:port or host:port:port " + " or host:port:port:type");
				}
				// tianyankee：当前节点的类型
		...
		if (!quorumEnableSasl && quorumServerRequireSasl) {
			throw new IllegalArgumentException(QuorumAuth.QUORUM_SASL_AUTH_ENABLED + " is disabled, so cannot enable "
		...
		// tianyakee：如果没有设置dataLogDir，则将dataLogDir设置为dataDir
		if (dataLogDir == null) {
			dataLogDir = dataDir;
		}
		if (clientPort == 0) {
		...
		if (servers.size() == 0) {
			...
			// tianyakee：在dataDir目录下写server的id
